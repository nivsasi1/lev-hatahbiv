  ...
  return (
    <div className={"category-page"}>
...

      {
        ...
        (tree?.subs?[currentSection] && tree.subs[currentSection].subAmount ?? 0 > 0 ?
        tree?.subs[currentSection].subs?.forEach((current, index) => {
          return (
            <div className={"products"}>{products.filter((product) => {product.sub_cat === currentSection && product.third_level === index})}</div>
        )}) : <></> )
...
const renderProducts: React.FC<{Array}> = ()=>{
