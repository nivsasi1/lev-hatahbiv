      ...
      fetch(`http://localhost:5000/product/${id}`, {
        ...
        .then((data) => {
          ...
          if (data.data.sub_cat) {
            setSubCat(
              SectionMenuList.find(
                (section) => section.title === data.data.category
              )?.options.map((option) => {
                return option.optionTitle;
              })
            );
            ...
            fetchTreeData().then((treeData) => {
              ...
              setThirdLevel(
                treeData.data.find(
                  (section: any) => section.sub_cat === data.data.sub_cat
                )?.third_level
              );
            });
          }
      ...
      <div className={"add-product-form"}>
        ...
        <div class="baka-input-wrapper">
          ...
          <Input
            ...
            type="string"
            check={
              (value) => Number(value) > 0 /*/^(\d+|\d+.\d+)$/.test(value)*/
            }
            ...
            <DropDown
              ...
              didSelect={(index) => {
                if (third_levelTree) {
                  const third_level_list = third_levelTree.find(
                    (section: any) =>
                      section.sub_cat === sub_cat[index as number]
                  )?.third_level;
